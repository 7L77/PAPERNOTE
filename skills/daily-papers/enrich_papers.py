...
def strip_tags(html: str) -> str:
    ...
    return re.sub(r"\s+", " ", html_unescape(re.sub(r"<[^>]+>", " ", html))).strip()
...
def extract_from_abs(html: str) -> dict:
    """Extract metadata from arxiv abs page meta tags and metadata table."""
    ...
    for m in re.findall(r'<meta\s+name="citation_author_institution"\s+content="([^"]+)"', html):
        if m.strip():
            affils.add(m.strip())
    comments = extract_abs_field(html, "Comments")
    journal_ref = extract_abs_field(html, "Journal reference")
...
async def enrich_one(paper: dict, sem: asyncio.Semaphore) -> dict:
    ...
    try:
        ...
        # Abs page metadata is cheap and also carries comments / journal-ref.
        abs_authors = []
        abs_affiliations = []
        abs_comments = ""
        abs_journal_ref = ""
        ...
        if abs_html:
            ...
            abs_comments = abs_data["comments"]
            abs_journal_ref = abs_data["journal_ref"]
            abs_preferred_venue = abs_data["preferred_venue_match"]
            abs_venue = abs_data["venue"]
        ...
        result["comments"] = abs_comments or paper.get("comments", "")
        result["journal_ref"] = abs_journal_ref or paper.get("journal_ref", "")
        result["preferred_venue_match"] = abs_preferred_venue or paper.get("preferred_venue_match", "")
        result["venue"] = abs_venue or paper.get("venue", "")
