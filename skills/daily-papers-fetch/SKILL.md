...
description: |
  ...
  也支持显式范围："2026年论文推荐"、"2026-01-01到2026-03-09论文推荐"
---
...
优先解析显式日期范围；只有未命中时再退回 `--days N`。匹配规则：
- `2026年论文推荐` → `--start-date 2026-01-01 --end-date 2026-12-31`，如果今年尚未结束，则结束日期取今天
- `2026-01-01到2026-03-09`、`2026/01/01 到 2026/03/09` → `--start-date 2026-01-01 --end-date 2026-03-09`
- "过去一周"、"最近7天"、"一周的论文" → `--days 7`
- "过去3天"、"最近三天"、"抓3天" → `--days 3`
- "过去两周" → `--days 14`
- 无特殊指定 / "跑一下论文抓取" → 不加 `--days`（默认当天）

将解析结果存为以下二选一变量，并在后续脚本调用中使用：
- `RANGE_ARGS="--start-date YYYY-MM-DD --end-date YYYY-MM-DD"`
- `DAYS_ARG="--days N"`
...
# 显式日期范围模式
python3 ../daily-papers/fetch_and_score.py --start-date YYYY-MM-DD --end-date YYYY-MM-DD > /tmp/daily_papers_top30.json
```

根据前面解析结果：
- 如果有 `RANGE_ARGS`，优先传 `--start-date ... --end-date ...`
- 否则如果有 `DAYS_ARG`，传 `--days N`
- 否则不加参数（默认当天）
...
显式日期范围模式的额外规则：
- 只抓取日期范围内的 HF Daily 和 arXiv
- **跳过 HF Trending**，因为该接口不支持历史日期
- **跳过 history 去重**，因为用户要看完整范围汇总
...
- 显式范围模式下没有 trending 兜底，这是为了保证时间范围语义准确
