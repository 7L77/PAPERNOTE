...
问题：
对“很接近的候选”排序不稳定
	NEAR 本质上主要反映的是**网络表达能力**，而**不直接刻画训练过程本身**，所以它可以预测大趋势，但对一些**很接近的方案之间的细微差别**，未必能分得很准。
	比如在激活函数和初始化方法比较中，它能稳定识别差方案，但对几个性能非常接近的好方案，排序未必完全一致。

1. 缺少 top-P 导向：不像 VKDNW 强调 nDCG/头部命中，NEAR 更偏全局相关。
2. 不含鲁棒性视角：相比 CRoZe，NEAR 没有 clean/perturb 一致性建模。
3. 深层退化未专门处理：不像 NCD 专门修 activation proxy 在深子空间的负相关问题。

计算加速：随机 SVD / 分层抽样 / 自适应 repetitions（不稳定层多算，稳定层少算）
鲁棒扩展：加入轻量扰动一致性项（借鉴 CRoZe 思路），形成 NEAR-R
深层修正：在激活矩阵构造前加随机 mask + 非线性 rescale（借鉴 NCD）缓解深层负相关。
...
超参数敏感
	超参敏感：μ/σ（size regularization）和 batch/采样策略会影响稳定性
...
仍需 alpha 调参，尚未“免调参”
跨架构范式（尤其非 CNN）外推还不稳。
仍是 activation-based 路线，没从根本上解决“单代理在异构空间失效”的问题。
